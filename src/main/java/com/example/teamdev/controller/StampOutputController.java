...
public class StampOutputController {
    ...
    private static final Logger logger = LoggerFactory.getLogger(StampOutputController.class); // Loggerを追加
    ...
    public String output(
        ...
        if (!bindingResult.hasErrors()) {
            try {
                ...
                model.addAttribute("errorMessage", "CSV出力処理中にエラーが発生しました。");
        ...
        model.addAttribute("result", "出力条件を確認してください。"); // 汎用的なメッセージ
        return view(model, session, redirectAttributes);
    }
    ...
    public String view(
        ...
        try {
            ...
            logger.error("打刻履歴出力画面の表示中に予期せぬエラーが発生しました。", e);
