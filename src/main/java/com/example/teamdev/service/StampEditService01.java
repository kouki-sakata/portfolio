...
public class StampEditService01{
	...
	public void execute(List<Map<String, Object>> StampEditList, int updateEmployeeId) {
		...
		for (Map<String, Object> stampEdit : StampEditList) {
			...
			String stInTime = stampEdit.get("inTime") != null ? stampEdit.get("inTime").toString() : null;
			String stOutTime = stampEdit.get("outTime") != null ? stampEdit.get("outTime").toString() : null;
            // カンマ区切り対策
			...
			if (stInTime != null && !stInTime.isEmpty()) {
		        // LocalDateTimeに変換
		        LocalDateTime localDateTime = parseToLocalDateTime(stYear,stMonth,stDay,stInTime);
	        ...
	        if (stOutTime != null && !stOutTime.isEmpty()) {
		        // LocalDateTimeに変換
	        	LocalDateTime localDateTime = parseToLocalDateTime(stYear,stMonth,stDay,stOutTime);
		        // LocalDateTimeからTimestampを作成
	        ...
	        if(timestampInTime != null) {
				if(timestampOutTime != null) {
					...
					int comparison = timestampInTime.compareTo(timestampOutTime);
					//出勤時刻>退勤時刻
					if (comparison > 0) {
						// 1日を加算
			        	LocalDateTime localDateTime = parseToLocalDateTime(stYear,stMonth,stDay,stOutTime);
	        ...
	        String idString = stampEdit.get("id") != null ? stampEdit.get("id").toString() : "";
	        Integer id = !idString.isEmpty() ? Integer.parseInt(idString) : null;
			//idが格納されている場合は更新
			if (id != null) {
	...
	public static LocalDateTime parseToLocalDateTime(String year, String month, String day, String time) {
        ...
        LocalDateTime localDateTime = LocalDateTime.parse(dateTimeString, DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm"));
