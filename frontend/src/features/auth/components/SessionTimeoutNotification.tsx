...
export const SessionTimeoutNotification = () => {
  const { sessionInfo, refreshSession, logout, sessionTimeoutWarning } =
    useAuth();
  ...
  return (
    <SessionTimeoutWarning
      ...
      onExtend={handleExtend} // ミリ秒を秒に変換
      onLogout={handleLogout}
      show={shouldShowWarning}
      timeRemaining={Math.floor((timeRemaining ?? 0) / 1000)}
      urgent={timeRemaining !== null && timeRemaining <= 60_000} // 1分以下で緊急
    />
  );
};
