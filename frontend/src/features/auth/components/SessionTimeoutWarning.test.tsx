...
describe("SessionTimeoutWarning", () => {
  ...
  describe("レンダリング", () => {
    it("警告メッセージを表示する", () => {
      ...
      expect(
        screen.getByText(/セッションの有効期限が近づいています/)
      ).toBeInTheDocument();
    });
    ...
    it("セッション延長ボタンを表示する", () => {
      ...
      expect(
        screen.getByRole("button", { name: /セッションを延長/ })
      ).toBeInTheDocument();
    });
  ...
  describe("アクセシビリティ", () => {
    ...
    it("緊急度に応じたaria-liveを設定する", () => {
      render(
        <SessionTimeoutWarning {...defaultProps} timeRemaining={30} urgent />
      );
  ...
  describe("スタイリング", () => {
    it("警告レベルに応じたスタイルを適用する", () => {
      const { rerender } = render(
        <SessionTimeoutWarning {...defaultProps} timeRemaining={300} />
      );
  ...
  describe("条件付きレンダリング", () => {
    it("showプロパティがfalseの場合は表示しない", () => {
      const { container } = render(
        <SessionTimeoutWarning {...defaultProps} show={false} />
      );
