...
describe("profileApi", () => {
  ...
  it("fetches profile statistics", async () => {
    mockedApi.get.mockResolvedValue(sampleStatistics);

    const result = await fetchProfileStatistics();

    expect(mockedApi.get).toHaveBeenCalledWith("/profile/me/statistics");
    expect(result).toStrictEqual(sampleStatistics);
  });
});
