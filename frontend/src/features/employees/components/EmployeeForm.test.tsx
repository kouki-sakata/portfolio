...
describe('EmployeeForm', () => {
  ...
  it('名が50文字を超える場合バリデーションエラーを表示する', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('名は50文字以内で入力してください')).toBeInTheDocument();
    });
  });
  ...
  it('更新モードではパスワードフィールドに適切なヒントを表示する', () => {
    ...
    expect(screen.getByText('空欄のままにすると、パスワードは変更されません')).toBeInTheDocument();
    expect(screen.getByPlaceholderText('変更する場合のみ入力')).toBeInTheDocument();
  });
  ...
  it('新規作成モードではパスワードフィールドに適切なプレースホルダーを表示する', () => {
    ...
    expect(screen.getByPlaceholderText('8文字以上で入力してください')).toBeInTheDocument();
  });
});
